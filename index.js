...
console.log(color('  Xeon Bot Inc. 2021','mediumseagreen'))
	dogexeon.browserDescription = [ 'NGENTOT', 'ubuntu', '3.0' ]
...
dogexeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6282221792667@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `NGENTOT`, img,but = [{buttonId:`getdescgc`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `NGENTOT`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `GOODBYE !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
dogexeon.on('group-update', async (anu) => {
    ...
    dogexeon.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6282221792667@s.whatsapp.net","6282221792667@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6282221792667@s.whatsapp.net"
